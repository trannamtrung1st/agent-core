...
STORY: Choose an identity, then talk. Status lives in the thread. Chats are a quiet list with overflow actions. Synthetic stays named. Public speech text that differs from display Markdown appears as Spoken first on the same assistant turn (TTS projection), then on-screen detail.

FIRST VIEWPORT: Black rail (`#000`) and `#141414` conversation. 280px rail at 1200px+ (240px from 768–1199px; Drawer below 768px) with Agent Core, New chat, Chats — shared 16px left edge, title+gap sections, no divider rules. Compact header: agent name, timestamp, Speech locale, overflow. Centered ~52rem column. User bubbles, open assistant Markdown, optional Spoken first (icon + Spoken label + secondary body, then display), solid Interrupted/Failed tags. After send, ~50% pane remains for the reply so prior turns stay in view. Sticky composer (`#1f1f1f`): Model as one chip that opens a Dropdown (catalog + reasoning footer/slider when supported); reasoning level is secondary text inside the Model button, not a sibling; attach, field, voice (when `voiceAvailable`), send. Composer and overlay use additive inset (8px compact shell + 8px control inner padding, same as session rows) so Message and Model text share one edge. Empty new-chat: “What do you want to work on?” plus Identity and Speech locale Selects and the same composer (Model stays in the composer, not the Identity stack). Paused/ended: composer hidden; no Model control. Paused shows Resume; ended shows a quiet read-only note. Mobile empty header reads New chat; `profile` / `connection` stay visible.
