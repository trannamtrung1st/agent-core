...
The overlay matches the native `http-openrouter` launch profile: hosted OpenRouter text and vision (`Adapter=OpenAICompatible`, `Vision=true` for the selected model). Hosted STT/TTS are not selected in this file; Real voice stays unavailable until those adapters are resolved at startup (`voiceAvailable` is false and Voice create returns 409). Native processes can pass the same text environment without Compose:
