...
**Compose and browser verification:** default Synthetic Compose smoke (`scripts/compose-sqlite-volume.sh`) proves SQLite volume survival for sessions, catalog, and redacted Background Work APIs. The P7 whole-phase Admin journey runs in Playwright with a **disposable** SQLite file (`PLAYWRIGHT_SQLITE_PATH`, isolated `admin-lifecycle` project in CI). Extending Compose to create/publish a managed definition and chat after container restart remains a W08 gate item ([P7G report](reports/p7g-history-rollback-final-gate.md)).
