...
`/health` reports `"profile":"Real"`. Compose interpolates a gitignored root `.env` (copy from `.env.example`) or the same variables from the host environment. Do not commit `.env`. Never put secrets in the Compose files. `OPENROUTER_API_KEY` is required to start the Real overlay. `AGENTCORE_LLM_MODEL` defaults to `deepseek/deepseek-v4.1-flash` (pinned OpenRouter model with tool calling and configurable reasoning; text input only); do not set it to `openrouter/free`. `AGENTCORE_LLM_REASONING_EFFORT` defaults to `medium` and maps to the adapter `reasoning_effort` request field. Pair capability overrides with the model: `AGENTCORE_LLM_TOOLS` defaults to `true` for the pinned model; `AGENTCORE_LLM_VISION` defaults to `false` (set `true` only when overriding to a vision-capable model such as `deepseek/deepseek-v4-flash-vision-exp`). Stop with the same files: `docker compose -f docker-compose.yml -f docker-compose.real.yml down`. Synthetic and Real share volume `agent-core-data`. `.env` is not an application `IConfiguration` source; native `dotnet run` does not load it unless you export those variables into the process environment.
