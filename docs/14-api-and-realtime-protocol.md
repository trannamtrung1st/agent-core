...
| POST /api/v2/admin/definition-drafts/{draftId}/publish | Owner capability; trusted local caller; `{ expectedRevision }` | 200 publication summary (`metadataRevision` included) | 400 validation or stale/missing required eval evidence; 401 capability; 403 non-local; 404; 409 stale revision |
