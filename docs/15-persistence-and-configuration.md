...
**Follow-on P1 planned until verified:** `LastEntrySequence` remains a first-class durable snapshot coordinate and must not be derived from `Entries[^1]` once `Entries` is a window. Bounded saves retain older `ConversationEntries` rows and must not renumber them. Runtime restore loads required prompt context, interrupted response/delivery coordinates, and the complete trailing unresolved user suffix without requiring the full transcript in memory. Ending recovery still maps Ending → Ended. Additive `lifecycleStatus`, purpose, deadline, completion policy, and session speech-locale override persist beside existing protocol-v1 `status` and `ArchivedAt`. See [Technology Decisions](10-technology-decisions.md#decision-bounded-history-and-durable-lastentrysequence).
