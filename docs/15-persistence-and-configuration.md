...
A reopened snapshot loads the newest restore window, not every raw row. Older conversation rows stay in the store. A committed summary is the model-facing copy of the closed prefix; compaction does not delete or rewrite those rows.
