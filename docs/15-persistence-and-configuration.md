...
Migration `20260925071140_P7DefinitionLifecycle` adds `AgentDefinitionDrafts` and `AgentDefinitionPublications`. Draft rows hold a mutable candidate JSON payload, monotonic draft `Revision`, fork source metadata, and timestamps. Revision-protected draft deletion removes the draft row plus unpublished resource bindings and evaluation scenario/result rows in one transaction; content-addressed blobs may remain for other bindings, and immutable publication rows are never removed. Publication rows store immutable definition JSON, server-assigned `(DefinitionId, Version)` unique across built-in file versions and durable rows, `MetadataRevision` for deprecation-only updates, and `Status` (`Active` / `Deprecated`). `IAgentDefinitionAdminStore` is implemented for InMemory (Synthetic) and SQLite (developer/Real). Runtime `IAgentDefinitionStore` composes file built-ins with durable publications without mutating `agents/*.json` or existing session snapshot definition JSON.

## P7C definition resource store (observed)

Migration `20260925084907_P7DefinitionResources` adds draft and publication resource binding rows (logical path, kind, media type, verified SHA-256, size) copied atomically inside the publish transaction. Resource mutations require the parent draft's expected revision and bump that revision. Content bytes live in `IDefinitionResourceContentStore` (InMemory for tests, file-backed blobs for SQLite). `AgentResourceLimits` enforce per-item, count, and aggregate caps; textual uploads run secret-pattern checks. Logical paths are bounded POSIX-relative names; traversal, repository-seed prefixes, and forbidden targets are rejected. Published bytes and bindings are immutable; later draft edits do not alter earlier publication hashes. Session `/workspace` remains the mutable session-owned area; publication template resources may seed `/workspace/working` once without write-back to definition storage.

## P7D managed instance store (observed)

Migrations `20260925101355_P7ManagedAgentInstance` and `20260925101918_P7ManagedAgentInstanceRevisionToken` extend `AgentInstances` for managed lifecycle: `Compatibility` distinguishes managed rows from per-definition compatibility aliases, `Lifecycle` (`Active` / `Archived`), monotonic instance `Revision`, and persona `PersonaRevision` with JSON persona payload. `IAgentInstanceStore.UpdateWithExpectedRevisionAsync` applies persona, active-version, and lifecycle mutations with stale-revision conflicts. Archive transitions update lifecycle metadata only; they do not delete conversation rows, structured memory, trigger registrations, or P6 `WorkItems`. Managed sessions persist `PinnedPersona` and `PinnedPersonaRevision` on `SessionSnapshot` at create time. InMemory and SQLite implementations share the same revision semantics for Synthetic and developer hosts.
