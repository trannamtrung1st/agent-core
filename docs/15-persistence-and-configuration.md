...
**Follow-on P1 planned until verified:** Additive `lifecycleStatus` and purpose/deadline/policy persist on the snapshot; Application `LifecycleTransition` is the persist path for pause/resume/terminal outcomes and deadline expiry. RequestComplete evaluation is observed. Session `SpeechLocaleOverride` persists independently of agent conversation language. See [Technology Decisions](10-technology-decisions.md#decision-provider-neutral-effective-speech-locale).
