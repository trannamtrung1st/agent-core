...
**Follow-on P1 planned until verified:** Additive `lifecycleStatus`, purpose, deadline, completion policy, and session speech-locale override persist beside existing protocol-v1 `status` and `ArchivedAt`. See [Technology Decisions](10-technology-decisions.md#decision-bounded-history-and-durable-lastentrysequence).
