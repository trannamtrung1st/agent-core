...
Do not mark remaining P1B/P1C gates observed until later batches implement them. Observed P1A coverage: long-history SQLite pages (newest/`before`/`after`, `before`+`after` 400, `hasOlder`, stable ids, attachments/artifacts/blocks/`SpeechText`/interrupted status) and frontend Load earlier messages with scroll-anchor preservation and stale-request cancel. Required coverage still planned: additive lifecycle transitions including RequestComplete vs RequestDeactivate, deadline expiry while attached and detached, Ending recovery; speech locale override/fallback without SessionRuntime vendor branches. Default suites stay Synthetic/offline. Real Chrome/Edge Voice and later non-English smoke remain distinct from fake-browser Playwright. See [Implementation Plan](18-implementation-plan.md#follow-on-p1-history-lifecycle-and-multilingual-speech-planned-until-verified).
