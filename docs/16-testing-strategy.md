...
Do not mark remaining P1C-3 manual non-English Browser smoke observed until that probe is recorded. Observed P1A coverage: long-history SQLite pages (newest/`before`/`after`, `before`+`after` 400, `hasOlder`, stable ids, attachments/artifacts/blocks/`SpeechText`/interrupted status) and frontend Load earlier messages with scroll-anchor preservation and stale-request cancel. Observed P1B coverage: additive lifecycle transitions including RequestComplete vs RequestDeactivate, deadline expiry while attached and detached, Ending recovery, and Playwright terminal UI for Completed/Expired/Cancelled (reload, lazy history, no reopen; paused Resume remains). Observed P1C-1: Application speech locale override > agent default > fallback, invalid-tag rejection, persistence/reconnect, public `capabilities.speechLocale`, and VoiceUnavailable while text remains. Observed P1C-2: Browser STT uses effective locale; Browser TTS exact/base/compatible or `SpeechVoiceUnavailable`; hosted batch STT language hint; hosted TTS locale/voice maps; realtime OpenAI STT stays unselectable. Observed P1C-3 UI: Speech locale Select, Playwright override/fallback, text composer remains when Voice is unavailable. Required coverage still planned: real non-English Browser STT/TTS smoke. Default suites stay Synthetic/offline. Real Chrome/Edge Voice and later non-English smoke remain distinct from fake-browser Playwright. See [Implementation Plan](18-implementation-plan.md#follow-on-p1-history-lifecycle-and-multilingual-speech-planned-until-verified).
