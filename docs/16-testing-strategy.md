...
SpeechFactory tests may register `OpenAiSpeechSynthesizer` with a dummy key to prove selection and credential gating; they must not send `audio/speech`. Live OpenAI TTS remains `AGENTCORE_LIVE_OPENAI_TTS=1` plus `OPENAI_API_KEY`. Missing speech keys must not fail Synthetic or Browser default suites.
