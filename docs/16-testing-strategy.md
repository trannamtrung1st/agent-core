...
Do not treat fake-device Playwright as the real Browser probes. P1C-3 real non-English Browser STT/TTS smoke is **observed** on HEAD `df0a12cecb5b60a12499488eed9c101cb01b45b2` (Chrome 153, `fr-FR`, native STT `Bonjour`, TTS `Daniel (French (France))`). The P0 `general-assistant` Voice checklist was re-run on that same HEAD and passed. Optional hosted multilingual checks were not run. **P1 is frozen**; P2 has not started.
