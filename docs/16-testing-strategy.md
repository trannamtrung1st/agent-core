...
P1-Final (2026-09-19, this run) repeated the live `.github/workflows/synthetic.yml` commands and `scripts/compose-sqlite-volume.sh` with live provider flags 0 and disposable Playwright SQLite roots: Domain 40; Infrastructure 112 passed / 9 skipped; Application 346 with `--blame-hang --blame-hang-timeout 5m` and no hang sequence; API 120; web 324 unit tests and production build; `CI=1` Playwright 34; Compose session `8e2467bf-7db9-4520-851a-146584761cb7` survived recreate. Catalog/archive/pause compatibility remains covered by `SessionCatalogTests` / `SessionCatalogApiTests` plus Playwright catalog and pause journeys. A broad `dotnet test AgentCore.sln` was not used.

Do not treat fake-device Playwright as the real Browser probes. P1C-3 real non-English Browser STT/TTS smoke is **observed** via unedited `p1-final-fr-smoke-r2.log`/`p1-final-fr-smoke-r2.json` pass on HEAD `5764010d989da965b252f5389e07669594c8bc29` (Chrome 153, `fr-FR`, native STT `Bonjour`, TTS `Daniel (French (France))`). The P0 `general-assistant` Voice checklist re-run is the unedited `p1-final-chrome-probe-run.log` pass on HEAD `df0a12c` (that same log's `non-english-browser-smoke` row remains fail). Original `p1-final-fr-smoke` remains fail. Optional hosted multilingual checks were not run. **P1 is frozen**; P2 has not started.

Observed P1A: long-history SQLite pages (newest/`before`/`after`, `before`+`after` 400, `hasOlder`, stable ids, attachments/artifacts/blocks/`SpeechText`/interrupted status) and frontend Load earlier messages with scroll-anchor preservation and stale-request cancel. Observed P1B: additive lifecycle transitions including RequestComplete vs RequestDeactivate, deadline expiry while attached and detached, Ending recovery, and Playwright terminal UI for Completed/Expired/Cancelled (reload, lazy history, no reopen; paused Resume remains). Observed P1C: Application locale precedence/validation, adapter wiring, Speech locale Select, Playwright override/fallback, text remaining when Voice is unavailable. Default suites stay Synthetic/offline. Real Chrome/Edge Voice and non-English smoke remain distinct from fake-browser Playwright. See [Implementation Plan](18-implementation-plan.md#follow-on-p1-history-lifecycle-and-multilingual-speech).
