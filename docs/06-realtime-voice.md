...
Before segmentation, `SpokenOutput` chooses the TTS source from the parsed envelope. That speakable string is the only TTS input: display Markdown is never narrated when a speech projection is used. Explicit `[[speech:]]` text is spoken as authored (subject only to a pathological runtime safety cap and dump guards). When speech is absent, conversational display prose is spoken in full with light markdown stripping; display and spoken text align semantically even when formatting markers differ. Whitespace-only display produces no spoken output. Visual-heavy display (fenced code, markdown tables, list-dominant reference material) and attachment or base64 dumps without an explicit speech projection use a short deterministic lead-in instead of narrating the visual material. List-heavy detection requires either an all-list block (three or more list lines and no other non-empty lines) or four or more list lines that make up at least half of non-empty lines, so numbered beats embedded in ordinary prose are not misclassified. Persisted envelope `SpeechText` is set when the model emitted `[[speech:]]` or when the runtime playback projection differs from `DisplayText` (structured lead-in, markdown stripping, or other coordinate changes). Plain display prose that matches the spoken string leaves `SpeechText` null. The model may supply `[[speech:]]` when the heard answer should intentionally differ from richer on-screen content. Segmentation then chunks the chosen speakable string for streaming latency; it does not truncate meaning.

## Post-MVP

Observed: optional independent `reply.speech` uses speech-coordinate playback receipts; display receipts stay on `reply.text` and blocks. Do not apply speech offsets to display text. TTS uses `[[speech:]]` when present as the intentional spoken projection; otherwise conversational display prose is spoken in full. Never narrate attachment bytes, AttachmentId dumps, or base64 extracts by accident; visual-heavy display without speech uses a short lead-in. Binary file upload remains HTTP, never the PCM path. Runtime deactivation tears down voice resources; reopen uses a fresh epoch and streamId. Text/voice mode switches keep the same Session identity, attachment/artifact refs, and response/epoch guards for late tool results.
