...
The default and primary MVP is **STT → Interaction Controller → text Agent Runtime / LLM → TTS**. Duplex is transport-specific. `serverAudio` is full-duplex from the user's perspective: microphone input and STT stay active during TTS playback even though reasoning is text-first, and it must never alternate between disabling capture to play an answer and recording the next turn. `clientTranscript` (Browser STT) is currently half-duplex: the local recognizer is suspended during agent output and does not send PCM to backend STT.
...
Raw microphone frames go directly from hub audio ingress to the active ISpeechRecognitionSession using a dedicated bounded channel. The ingress coordinator validates attachment/stream identity and serializes audio plus boundary markers. It emits only meaningful recognition/boundary/failure events into the session mailbox. An audio queue holds at most 500 ms; overflow or a sample gap terminates that input stream with recoverable AudioDiscontinuity. Restart recognition with a new streamId; do not silently splice missing speech. Maximum utterance is 120 seconds (`Voice.MaxUtteranceSeconds` default), after which force an ended boundary through ingress, restart recognition with a new streamId, and notify the client.
...
When input transport is `clientTranscript`, Voice mode becomes Listening without opening a backend `ISpeechRecognizer` session. Transcript evidence arrives as `client.speech.evidence`; `TryAdmitAudio` ignores PCM. The local recognizer is suspended during agent output (half-duplex); this is not the `serverAudio` full-duplex capture path. Native Web Speech `onstart`/`onend` are recognition-service session events, not user-utterance boundaries. Map `onspeechstart` to Agent Core `started` (falling back to the first result). Treat `onspeechend` as a **pending** boundary only: a native `result`, including a final, may arrive afterward, so close the utterance after the trailing final or a short grace timer (300 ms implementation heuristic; Web Speech does not bound this delay), and flush that pending turn on native `onend` only when `speechend` is already pending. Native `onend` without a pending `speechend` is service disconnect only: keep the Agent Core utterance open, notify `onRecognitionEnded`, and let the accumulator's bounded `unexpectedRestart()` freeze a `restartPrefix` and word-boundary-merge post-restart native finals/interims onto it (continuation-only or repeated-prefix results); normal in-session stable chunks keep space concatenation. Emit native `isFinal` chunks as `final` so the long-utterance accumulator can concatenate them. Unexpected native `onend` restarts the recognition service immediately when no application utterance is open (normal idle listening). While an application utterance is open, use bounded restart/backoff with a cap on consecutive service ends without `speechstart`/`result` progress; exceed that cap with `SpeechRecognitionRestartLimit` while the session remains voice, unmuted, and current-epoch. The transcript accumulator applies a separate bounded `unexpectedRestart()` cap for mid-utterance stitching. Observed Browser STT uses `speechLocale.effective` as native `lang`. Browser TTS applies `speech.output.segment` `language` and `speakingRate` to `SpeechSynthesisUtterance` and selects exact locale, then base language, then a compatible configured/default voice; when installed voices exist and none match, it fails with `SpeechVoiceUnavailable` instead of speaking English. `BrowserSpeechSynthesizer.speak` resolves when that utterance ends (or fails on `onerror`); only explicit cancel calls `speechSynthesis.cancel()`. Observed hosted batch STT posts an adapter-local Whisper language hint from the effective locale; hosted TTS locale/voice maps stay in adapters. SessionRuntime has no Browser/OpenAI locale branches. Realtime OpenAI STT remains unselectable. No automatic language detection. Observed Speech locale Select sets the session override. Real Chrome 153 `fr-FR` Browser STT/TTS smoke is observed (unedited `p1-final-fr-smoke-r2`: STT `Bonjour`; TTS Daniel French France) on HEAD `5764010`. See [Technology Decisions](10-technology-decisions.md#decision-provider-neutral-effective-speech-locale).

On interruption, persist full generated text internally, receivedTextEndExclusive for the public history projection, and heardTextEndExclusive for future context. Record the response's **delivery mode** (`text` or `voice`) on the conversation entry. The prompt uses heard prefix for voice-delivered assistant entries and received prefix for text-delivered entries, plus an application-owned interruption note, never the unplayed/unrendered tail. Unseen/unheard assistant tails must not enter future model context after a later mode switch. Public assistant history is the received display prefix in both modes, conservatively zero if none; `response.received` at final render normally credits the whole completed display. Voice playback acknowledgements update heard only and must not be copied onto received.
...
## Spoken output selection

In **Voice** mode, the runtime does not speculatively narrate display text while the model is still streaming. TTS waits for an accepted speech projection: explicit `[[speech:...]]` text (published live on `agent.speech.projection` as soon as the marker is complete) or, only after model completion, a one-shot compatibility fallback derived from final display text via `SpokenOutput.ForPlayback`. Display Markdown, blocks, tables, and code remain screen-only once Voice semantics apply; they never enter the speech pipeline after a projection is locked. That speakable string is the only TTS input. Explicit `[[speech:]]` text is spoken as authored (subject to a pathological runtime safety cap and dump guards). The compatibility fallback applies only when the final display text is **ordinary prose or ordinary Markdown** end-to-end. It does not stitch disconnected prose fragments around fences, tables, diagrams, or indented code into a synthetic narration. When display text contains materially rich structure, the runtime resolves no-speech unless explicit `[[speech:]]`/`speechText` is present. For wholly ordinary display, the fallback derives speakable prose after conservative sanitization (no runtime-authored summaries or lead-ins). Rich visual answers without explicit speech therefore stay silent rather than narrating diagrams, payloads, or intro/outro lines bracketing structured blocks. P2B's validated envelope (`displayText` required, optional `speechText`) is the intended long-term replacement for this compatibility path. It is intentionally higher latency than streaming display-derived narration. When no safe speakable prose remains, TTS stays silent and existing degraded-output handling applies. Text mode keeps display output as the normal conversational projection and does not require `[[speech:]]` unless the model already emits one. Persisted envelope `SpeechText` is set when the model emitted `[[speech:]]` or when the runtime playback projection differs from `DisplayText`. **Markers are temporary compatibility.** P2B replaces them with a validated provider-neutral envelope (`displayText`, optional `speechText`, optional `blocks`). `speechText` is optional even in Voice; absent speech uses validated display as the spoken fallback after completion. Agent definitions do not own TTS, UI layout, or marker syntax. `PromptContextBuilder.VoiceModeOutputGuidance` explains the temporary speech/display capability: Voice speech should be conversational, convey the point, and refer to specific on-screen detail when useful instead of narrating a long display answer. It stays framework-owned and does not impose an agent persona or fixed spoken length. The runtime does not rewrite or rebalance channels. Segmentation chunks the locked speakable string for streaming latency; it does not truncate meaning.

## Post-MVP

Observed: optional independent `reply.speech` uses speech-coordinate playback receipts; display receipts stay on `reply.text` and blocks. Do not apply speech offsets to display text. Voice TTS uses the accepted speech projection only (`[[speech:]]` live, or completion fallback). Never narrate attachment bytes, AttachmentId dumps, or base64 extracts by accident. Binary file upload remains HTTP, never the PCM path. Runtime deactivation tears down voice resources; reopen uses a fresh epoch and streamId. Text/voice mode switches keep the same Session identity, attachment/artifact refs, and response/epoch guards for late tool results.
