...
When input transport is `clientTranscript`, Voice mode becomes Listening without opening a backend `ISpeechRecognizer` session. Transcript evidence arrives as `client.speech.evidence`; `TryAdmitAudio` ignores PCM.
