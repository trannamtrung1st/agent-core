...
In **Voice** mode, the runtime does not speculatively narrate display text while the model is still streaming. TTS waits for a validated semantic speech projection (`speech.mode`): `custom` uses authored `speech.text`; `same` uses completed display after `SpokenOutput.ForPlayback` deterministic cleanup; `none` synthesizes nothing. When authored `custom` speech fails `SpokenOutput.ForPlayback` validation, the effective envelope normalizes to `same` with a safe display-derived playback coordinate when one exists, otherwise `none`; runtime-derived playback is never exposed as public custom `speechText`. Display Markdown, blocks, tables, and code remain screen-only once Voice semantics apply; they never enter the speech pipeline after a projection is locked. That speakable string is the only TTS input. Transient `agent.progress` (attachment/tool/status copy such as “Running tools…” or “Finalizing response…”) is never TTS or `clientSpeech` input; it is live UI status only. Provider reasoning stays off progress, display, speech, and history. Compatibility `[[speech:]]` markers exist only inside Infrastructure unstructured parsing and never in Application prompting. When no safe speakable prose remains, TTS stays silent and existing degraded-output handling applies. Text mode keeps display as the normal conversational projection. The first-party UI labels differing text-delivered speech as **Speech text** and Voice-delivered speech as **Spoken**. Persisted envelope `SpeechText` is public when custom speech differs from display. Agent definitions do not own TTS, UI layout, or marker syntax. `PromptContextBuilder.VoiceModeOutputGuidance` stays framework-owned Voice behavior (conversational spoken point, refer to on-screen detail) without marker instructions. Segmentation chunks the locked speakable string for streaming latency; it does not truncate meaning.

## Post-MVP

Observed: optional independent `reply.speech` uses speech-coordinate playback receipts; display receipts stay on `reply.text` and blocks. Do not apply speech offsets to display text. Voice TTS uses the validated speech projection only (`custom` text, `same` playback cleanup, or `none`). Never narrate attachment bytes, AttachmentId dumps, or base64 extracts by accident. Binary file upload remains HTTP, never the PCM path. Runtime deactivation tears down voice resources; reopen uses a fresh epoch and streamId. Text/voice mode switches keep the same Session identity, attachment/artifact refs, and response/epoch guards for late tool results.
