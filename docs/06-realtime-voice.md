...
Raw microphone frames go directly from hub audio ingress to the active ISpeechRecognitionSession using a dedicated bounded channel. The ingress coordinator validates attachment/stream identity and serializes audio plus boundary markers. It emits only meaningful recognition/boundary/failure events into the session mailbox. An audio queue holds at most 500 ms; overflow or a sample gap terminates that input stream with recoverable AudioDiscontinuity. Restart recognition with a new streamId; do not silently splice missing speech. Maximum utterance is 30 seconds, after which force a boundary and notify the client.
