...
## P5 triggers (observed)

`TriggerSchedulerHostedService` scans due registrations and then routes once. Schedule tools are `trigger.schedule_once`, `trigger.schedule_recurring`, `trigger.list`, `trigger.update`, and `trigger.cancel`. The host passes `ITriggerRegistrationService` into `SessionToolExecutor`. The model cannot set owner, provenance, or authorization origin. Occurrence evidence is clipped user-role data. Meter `AgentCore.Runtime` records `trigger_scheduler_events`, `trigger_registration_events`, and `trigger_due_lag_ms` without intent or evidence text. See [Persistence](15-persistence-and-configuration.md#p5-trigger-store-observed).

## P6 durable work (observed)

After `RouteOnceAsync`, `TriggerSchedulerHostedService` calls `DurableWorkIntake.AcceptAwaitingAsync` and then `DurableReminderExecutor.ExecuteDueAsync` on the existing one-second cadence. That is not a second scheduler. Waiting and terminal rows hold no claim. `work_events` records only an `outcome` tag. Logs record counts and exception type. See [P6 freeze candidate](reports/p6-freeze-candidate.md).

## P7B definition lifecycle (observed)

`AgentDefinitionLifecycleService` owns fork/create, exact-revision draft update/delete, validated publish, publication list, and metadata-only deprecate. Draft delete removes only unpublished draft state (including draft resource bindings and evaluation evidence), records `DraftDeleted` history atomically, and never removes immutable publications. `IAgentDefinitionAdminStore` persists drafts and publications (InMemory for tests, SQLite `AgentDefinitionDrafts` / `AgentDefinitionPublications` in production paths). `CompositeAgentDefinitionStore` implements `IAgentDefinitionStore` by overlaying durable publications on `FileAgentDefinitionStore` without changing Session Runtime call sites. Owner Admin lifecycle HTTP maps in `AgentCore.Api/AdminEndpoints.cs`: draft create/fork/list/read/update/delete/publish under `/api/v2/admin/definition-drafts` (including `DELETE .../{draftId}?expectedRevision=` and `POST .../{draftId}/publish`), and publication list/deprecate under `/api/v2/admin/definitions/{definitionId}/publications` (including `POST .../{version}/deprecate`). All routes share the owner-protected `/api/v2/admin` group. Provider DTOs and raw persistence records do not cross into Application or Domain. Candidate validation stays in Application with a pure Domain structural pass. See [Interfaces](04-backend-interfaces.md#p7b-definition-lifecycle-observed), [Persistence](15-persistence-and-configuration.md#p7b-definition-lifecycle-store-observed), and [P7B report](reports/p7b-definition-lifecycle.md).

## P7C harness resources (observed)

`AgentDefinitionResourceService` and `IAgentDefinitionResourceAdminStore` manage draft resource bindings with expected draft revision, immutable publication snapshots, and `IDefinitionResourceContentStore` blobs. `DefinitionPublicationResourceReader` feeds `FileSessionWorkspace` read-only `/agent/resources` paths; template-kind resources copy into `/workspace/working` on first `EnsureAsync` when `WorkspacePolicy.TemplateId` matches. Admin resource HTTP (list/upload/bind/remove/content, publication list) and `GET /api/v2/admin/tools` live in `AdminEndpoints.cs`. Admin UI draft tabs cover Instructions, Capabilities (`RoleEnvironment`), and Resources. See [Interfaces](04-backend-interfaces.md#p7c-definition-resources-observed), [Persistence](15-persistence-and-configuration.md#p7c-definition-resource-store-observed), and [P7C report](reports/p7c-harness-resources-workspace.md).

## P7D managed instances (observed)

`AgentInstanceService` implements managed create, persona update, lifecycle archive/unarchive, and version reassociation with optimistic `Revision` / `PersonaRevision` checks. `AdminEndpoints.cs` maps `POST /api/v2/admin/agent-instances` and `PATCH .../persona`, `PATCH .../lifecycle`, and `PATCH .../active-version` with strict JSON contracts (`AdminAgentInstancePersonaHttp`, lifecycle and version DTOs). v2 `SessionCreateHttp` routes mutually exclusive `agentInstanceId` (managed pin path) or legacy `agentId` (compatibility). `SessionRuntime` `BuildReady` and `PublicHistory.FromSnapshot` project pinned persona into `session.ready` and public history for reopened managed chats. Trigger admission paths reject archived managed owners at schedule create and due execution. See [Interfaces](04-backend-interfaces.md#p7d-managed-instances-observed), [Persistence](15-persistence-and-configuration.md#p7d-managed-instance-store-observed), [Protocol](14-api-and-realtime-protocol.md), and [P7D report](reports/p7d-managed-instance-identity.md).

## P7E memory and automation Admin (observed)

`AdminMemoryService` and `AdminAutomationService` expose owner-protected list/delete/reset and trigger registration cancel through existing P4/P5 stores with scope confirmation and revision checks. `TriggerInstancePolicyReconciliationService` suspends ineligible future registrations on managed version downgrade or archive without cancelling accepted P6 work. HTTP routes under `/api/v2/admin/agent-instances/{instanceId}/learned-memory` and `.../automation/...` map in `AdminEndpoints.cs`. See [Interfaces](04-backend-interfaces.md#p7e-memory-and-automation-admin-observed) and [P7E report](reports/p7e-memory-automation-admin.md).

## P7F validate, evaluate, diff, publish (observed)

`AgentDefinitionDraftValidationService`, evaluation scenario store/runner, `AgentDefinitionDraftDiffService`, and `AgentDefinitionDraftPublishService` implement the exact-revision publish gate with `configurationFingerprint`, `scenarioVersion`, and required Synthetic evidence checks. Admin HTTP covers validate, evaluation CRUD/run/results, diff, and publish on `/api/v2/admin/definition-drafts/{draftId}/...`. See [Interfaces](04-backend-interfaces.md#p7f-validation-evaluation-and-publish-gate-observed) and [P7F report](reports/p7f-validation-evals-publish-gate.md).

## P7G Admin history (observed)

`IAdminEventStore` append-only persists allowlisted lifecycle events with `OperationId` idempotency; mutators record events in the same transaction as durable Admin mutations at the store boundary. `GET /api/v2/admin/events` lists safe projections only. Docker Compose persists SQLite and `Persistence__DefinitionResourceRoot=/data/definition-resources` on the same volume so Admin publication resource bytes survive container recreate (`scripts/compose-sqlite-volume.sh`). See [Persistence](15-persistence-and-configuration.md#p7g-admin-events-observed), [Operations](17-observability-and-operations.md#docker-compose-integration-and-demo), and [P7G report](reports/p7g-history-rollback-final-gate.md).

## Post-MVP

Observed: owner-capability filter; catalog/lifecycle in the revision stream; `IAttachmentStore` HTTP streamed pending→bound uploads with opaque blobs outside SQLite and outside `local/`; bind after durable user entry; speech-staged pending files; TTL and durable-delete cleanup. Observed `IAttachmentProcessor` runs off the mailbox with 256 KiB / 10 s / 256 MiB / 32 MP fail-closed limits, AttachmentId+version cache, no remote fetch, and typed unsupported results. Stale off-mailbox `AttachmentsProcessed` completions for superseded turns do not launch brain work; idle accounting pairs the extraction `BeginWork()` with a single dispatch `EndWork()`; `processingAttachments` output clears on stale completion only when that response still owns attachment progress. Production OpenAI-compatible adapters report `Vision` honestly and map normalized image parts or return `UnsupportedCapability`. Observed rich envelope persistence: `ConversationEntries.EnvelopeJson` stores display/speech/blocks and delivery flags atomically with status and heard/received offsets. TTS waits for a validated semantic speech projection (`speech.mode`): `custom` uses authored `speech.text`; `same` uses completed display after `SpokenOutput.ForPlayback` deterministic cleanup; `none` synthesizes nothing. Compatibility `[[speech:]]` markers exist only inside Infrastructure unstructured parsing and never in Application prompting or SessionRuntime generation; the first `[[speech:…]]` marker in stream order wins (including `[[speech:none]]`) so early compatibility speech publication cannot be revised by a later marker. Completion fallback never invents runtime-authored spoken prose; dump-like display yields no speakable fallback. Public history projects only the received display prefix and display-delivered blocks. Attachment refs authorize ids bound on conversation entries plus `fixture-attachment-1`; artifact refs authorize stored ArtifactIds plus `fixture-artifact-1`. Observed `ISessionWorkspace`: lazy provision, logical execution view, 250 MiB `/workspace` writes, template copy without secret/corpus injection, durable-delete cleanup. Observed `IArtifactStore`: binaries under `data/artifacts`, 50 MiB each / 250 MiB per session, explicit attachment materialize with hash/provenance. Observed typed tools: Application `SessionToolExecutor` scoped capabilities (knowledge, AttachmentId reads, logical workspace read/list/write/patch, artifacts including `artifacts.create_from_workspace`, optional `sandbox.run`, bounded `web.search`/`web.fetch`, and `email.*`); 12 / 30 s / 120 s / 8 MiB caps; `runningTools` initiative hold; late/stale results rejected; OpenAI-compatible tool_calls mapping with offline adapter tests. `ToolRegistry`/`ToolEffect` and execution-time allowlist rechecks gate every call before integration access; `RequireApproval` sensitive writes pause the 30 s/120 s execution clocks until `agent.approval.respond` on the owning response, then start a fresh per-tool timer. `FileSessionWorkspace.PatchTextAsync` applies edits in memory then commits via atomic replace. Public web uses Infrastructure `PublicWebFetcher`/`SocketsPublicWebTransport` with per-connect DNS validation and SSRF denials; Synthetic/Brave search providers register by profile. Email uses Synthetic or `GmailEmailProvider` with MimeKit draft MIME, Bcc-preserving raw reread (fail-closed without RFC822), draft-hash-gated send, and Gmail `drafts.send` with approved `message.raw`; indeterminate send outcomes consume the approval. Web tool text is ephemeral to the current request and is not written into `ConversationEntries`. Scripted Support/Compliance workflows emit concise text plus Markdown and artifact refs. Uploads are never executed. No process tool by default. **Phase I not-applicable:** Support, Compliance, and `sandbox.run` still have no durable WorkItems after `RequestDeactivate`. P6 WorkItems are a separate observed path for trigger occurrences. The future Phase I trigger remains in [Technology Decisions](10-technology-decisions.md#post-mvp-planned-until-verified).
