...
```json
{
  ...
  "initiativePolicy": {"enabled": true, "silenceThresholdMs": 10000, "cooldownMs": 30000, "maxPerSilencePeriod": 5, "maxConsecutiveProactiveTurns": 5, "triggers": ["longSilence", "environmentUpdate", "unfinishedInteraction"]},
  "voice": {"enabled": true, "voiceId": "default", "speakingRate": 1.0},
  "providerPreferences": {"languageModel": "primary-llm", "speechRecognizer": "primary-stt", "speechSynthesizer": "primary-tts", "interruptionClassifier": "heuristic"},
...
IAgentBrain checks initiative eligibility and contextual usefulness. Default rule: LongSilence returns Speak when definition consecutive/silence caps still allow proactive speech and the session already has at least one completed assistant turn (so the agent is continuing while the user is inactive, not opening a cold session); EnvironmentUpdate speaks only for an allowlisted meaningful status change relevant to this session; UnfinishedInteraction speaks only if a pending topic exists. Exhausted non-zero consecutive, silent-evaluation, or inactivity bounds return RequestDeactivate; zero cap returns StaySilent until those other bounds apply. Otherwise return StaySilent. UserTurn normally returns Speak. The controller rechecks eligibility after asynchronous decisions return and enforces hard runtime caps even when the brain returns Speak. Observed: repeated Speak in one silence period is allowed when `maxPerSilencePeriod` and `MaxConsecutiveProactiveTurns` permit it; `DefaultAgentBrain` and stand-in test brains cover distinct caps.
...
Decision: Infrastructure/OpenAICompatibleLanguageModel implements ILanguageModel using IHttpClientFactory, System.Text.Json and direct HTTP, no vendor SDK. The preferred hosted configuration targets OpenRouter, while direct OpenAI or local compatible inference use the same adapter. Named configuration supplies BaseUrl, ApiKey, DefaultModel, AdditionalHeaders, Timeouts and capabilities. For the initial hosted **adapter smoke** configuration only, DefaultModel may be `openrouter/free`. The Real/demo profile must set a **fixed** operator-selected OpenRouter model ID; do not use the random free-model router for product demos. Bind ApiKey from `OPENROUTER_API_KEY` or equivalent backend configuration; never from the browser. BaseUrl is the API root ending in `/v1/`; append `chat/completions` without resetting an existing path prefix. Reject non-http(s) URLs and embedded userinfo; allow local HTTP only for explicitly configured demo/self-hosted endpoints. Values come from trusted backend operator configuration, never browser requests.
