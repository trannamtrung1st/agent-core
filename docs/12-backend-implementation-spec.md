...
IAgentBrain checks initiative eligibility and contextual usefulness. Default rule: LongSilence speaks only if the last delivered assistant turn ends in a question and definition consecutive/silence caps still allow Speak; EnvironmentUpdate speaks only for an allowlisted meaningful status change relevant to this session; UnfinishedInteraction speaks only if a pending topic exists. Exhausted non-zero consecutive, silent-evaluation, or inactivity bounds return RequestDeactivate; zero cap returns StaySilent until those other bounds apply. Otherwise return StaySilent. UserTurn normally returns Speak. The controller rechecks eligibility after asynchronous decisions return. Observed: repeated Speak in one silence period is allowed when `maxPerSilencePeriod` and `MaxConsecutiveProactiveTurns` permit it and each eligible LongSilence check still sees a question-ending last assistant turn; `DefaultAgentBrain` and stand-in test brains cover distinct caps.
