...
**Roadmap handoff:** **P4** (context compaction and memory). P4A compaction is implemented: a valid summary boundary, a semantic candidate that does not rewrite raw rows, and a mailbox commit that survives reopen. P4B session-scoped structured memory has a store and trusted service; prompt projection, definition policy, and the P4B checkpoint are not started. P4C is not started. This is not a P4 freeze. **P3 key-free freeze** on `4dbb920` (verified hosted descendant `ac795b6`, workflow `35682808408`). See [P3 closure report](reports/p3-freeze-candidate.md).
