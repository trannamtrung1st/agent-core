...
| --- | --- | --- |
| P1A history | Evolve `IMemoryStore`; durable `LastEntrySequence` independent of the `Entries` window; retain older rows; messages newest/`before`/`after` with `hasOlder`; one public history projection; UI Load earlier messages with scroll-anchor preservation | **P1A-1 store restore/metadata observed** on SQLite/in-memory. Remaining: HTTP `before`/newest paging; frontend/Playwright long-history |
