...
| P1-Final | Exact live `synthetic.yml` plus Compose; catalog/archive/pause regressions remain; P0 Chrome Voice checklist re-run | **Frozen** on HEAD `15930985f54e2e6bf4019dd0d8040796883021c7` after 2026-09-19 repair: Domain 40; Infra 112/9 skip; App 356 blame-hang; API 123; Vitest 328; Playwright 34 (dedicated `data/playwright/synthetic.db`); Compose-equivalent volume smoke on :5088 (exact script blocked by Real :5080). Unedited Voice re-run: `p1-repair-chrome-probe-run.log` (queued-text pass; combined `non-english-browser-smoke` remains fail). Unedited `fr-FR` smoke: `p1-repair-fr-smoke-r2` pass. Earlier probe records stay as written. P2 not started |
