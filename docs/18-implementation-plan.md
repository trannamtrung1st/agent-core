...
**Roadmap handoff:** **P4** (context compaction and memory). P4A compaction is implemented: a valid summary boundary, a semantic candidate that does not rewrite raw rows, and a mailbox commit that survives reopen. P4B session-scoped structured memory has a local checkpoint: five kinds, correction, content-free deletion, profile precedence, session isolation, SQLite reopen, prompt projection, and durable delete. P4C-0 keeps a durable agent instance separate from the reusable definition. Historical sessions keep their pinned definition. IdentityUser promotion copies a session item for one instance and trusted profile. User-scope memory copies a Session or IdentityUser item for one trusted profile and is retrieved only by definitions that enable it. P4 has a local closure candidate in [the P4 report](reports/p4-freeze-candidate.md). Hosted CI was not re-run for that candidate. **P3 key-free freeze** on `4dbb920` (verified hosted descendant `ac795b6`, workflow `35682808408`). See [P3 closure report](reports/p3-freeze-candidate.md).
...
**P4 local closure candidate** (not a hosted workflow): `npm ci` in `tests/realtime-js`; Domain 77 passed; Infrastructure 267 passed / 13 skipped; Application 594 passed / 1 skipped with `--blame-hang --blame-hang-timeout 5m` and no hang sequence; API 167 passed; web `pnpm install --frozen-lockfile`, Vitest 50 files / 392 passed, production build; `pnpm exec playwright install chromium --with-deps` exited 0; `CI=1 pnpm exec playwright test` 47 passed; `./scripts/compose-sqlite-volume.sh` passed (`compose sqlite volume check passed`). Those commands ran on the working tree that already contained `P4ClosureTests` and the TODO/README status updates, and before this paragraph and `docs/reports/p4-freeze-candidate.md`. Hosted provider probes were not run.
