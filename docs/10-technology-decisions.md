...
P5 stores owner-scoped schedule registrations and normalized occurrences in `ITriggerStore`. The owner is the Agent Instance plus the trusted profile. The source session is provenance. An explicit current user turn may create, list, update, or cancel an eligible schedule without a second approval dialog. Initiative, environment, occurrence, history, and memory cannot authorize that write. A pending proposal from those sources survives only for one immediate confirming user turn. Creating a schedule does not approve a later sensitive tool such as `email.send`. Definitions without `TriggerPolicy` cannot schedule. `general-assistant` v8 may schedule; `customer-support` v2 may admit application events only. The hosted scheduler uses `TimeProvider`. Delivery is at-least-once with a stable occurrence id. One compatible live runtime may accept the occurrence. Otherwise the row stays `AwaitingDurableWork`. P5 does not implement the WorkItem engine. `DurableWorkHostedService` accepts each `AwaitingDurableWork` occurrence as one WorkItem and runs due work on a separate cadence from P5 scheduling. P6 freeze candidate `aeefffc` pending exact-SHA gate (core repair `2067a44`, workflow `36031813141` green). P7 follows recorded P6 freeze. Evidence: [P6 closure report](reports/p6-freeze-candidate.md). Evidence: [P5 closure report](reports/p5-freeze-candidate.md).
