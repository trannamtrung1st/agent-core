...
| P2D session model | Trusted catalog, system default, persisted per-session resolved choice, session-aware resolver, reasoning effort | Existing sessions stay pinned when the operator default changes | Implement in P2D ([decision](#decision-session-model-selection-and-inference-controls)); **planned** until verified |
...
**Status:** **Planned** as P2D. Do not treat this decision as observed until implementation and the key-free gate are complete. P1 remains frozen on `dceaccb`.
