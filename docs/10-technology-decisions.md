...
**Status:** **Observed** for bounded `IMemoryStore` restore, HTTP newest/`before`/`after` paging, and first-party Load earlier messages. Remaining follow-on P1 work (lifecycle, speech locale) stays planned until verified.
...
**Status:** **Planned until verified.** Observed P1B-1: purpose, completion policy, and additive `lifecycleStatus` persist with protocol-v1 `status` and orthogonal `ArchivedAt`. Authoritative transitions, deadlines-in-runtime, and RequestComplete remain unimplemented.
