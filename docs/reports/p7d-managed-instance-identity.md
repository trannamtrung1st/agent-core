...
**Gate HEAD:** `b082ece6274aa44eade84f6e7dc2286f1db63b9b` (W04 slice gate: docs on `cad4dab` product baseline plus P7D regressions)
...
| P7D ownership/archive regressions | `dotnet test tests/AgentCore.Application.Tests --filter FullyQualifiedName~ManagedInstanceP7DRegressionTests` | Pass (5) |
...
| AC-P7D-11 | New instance does not inherit another instance's learned memory/triggers | `Two_managed_instances_do_not_share_identity_user_memory` (`TrustedIdentityUserOwner` promotion + search, InMemory/SQLite); `Two_managed_instances_do_not_share_trigger_registrations`; session-scoped isolation in `Two_managed_instances_do_not_share_session_scoped_memory` |
...
| AC-P7D-14 | Archiving does not delete completed history/memory/trigger history | `Archived_managed_instance_retains_session_memory_and_trigger_history` retains session rows, `IdentityUser` learned memory (`SearchIdentityUserAsync`), and completed trigger registrations (InMemory + SQLite); journey readable histories |
