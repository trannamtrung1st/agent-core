...
Accepted product behavior beyond historical MVP: multiple persistent chats with agent selection; file/image attachments; rich replies; versioned role environments (Support, Compliance, examiner); durable isolated session workspaces; session-owned artifacts with explicit attachment materialization; bounded typed tools for Support/Compliance; Docker `sandbox.run` behind the same capability boundary (not host process/shell). Phase I WorkItems are not-applicable until a later accepted workflow must survive runtime deactivation. Details: [Technology Decisions](10-technology-decisions.md#post-mvp-planned-until-verified) and [Implementation Plan](18-implementation-plan.md#post-mvp-phases-planned-until-verified).
