...
Application ports `IDefinitionResourceContentStore` and `IAgentDefinitionResourceAdminStore` own bounded draft/publication resource bindings and verified content blobs. `AgentDefinitionResourceService` coordinates draft revision checks, upload/bind/remove, and publication read APIs. `DefinitionPublicationResourceReader` resolves exact `(definitionId, version)` bindings for `FileSessionWorkspace` `/agent/resources` projection and optional one-time template seeding into `/workspace/working`. `IAgentInstanceService.CreateAsync` plus `SessionManager.CreateForInstanceAsync` pin managed sessions to the instance's active publication and persona; v2 session create accepts mutually exclusive `agentInstanceId` or legacy `agentId`. Infrastructure implements InMemory and SQLite stores with migration `20260925084907_P7DefinitionResources`. See [Persistence](15-persistence-and-configuration.md#p7c-definition-resource-store-observed) and [P7C report](reports/p7c-harness-resources-workspace.md).
