...
Observed email port: Application `IEmailProvider` (`email.search`/`read`/`create_draft`/`send`) with Synthetic fixtures under Profile=Synthetic and `GmailEmailProvider` under Profile=Real using refresh-token OAuth (`Gmail:*` or `GMAIL_*` environment keys). `email.send` requires live approval bound to a draft content hash; provider failures and indeterminate transport outcomes surface as normalized tool results without leaking secrets.
