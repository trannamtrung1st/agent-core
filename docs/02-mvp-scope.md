...
**Historical MVP wording above remains the MVP acceptance record.** Observed post-MVP trusted-local owner capability ([R1](10-technology-decisions.md#decision-trusted-local-owner-capability-r1)) is a loopback/local-owner check for catalog, hub attach, uploads, and content. It is not tenant isolation, OAuth, or public multi-user hosting, and it does not rewrite this MVP exclusion.
