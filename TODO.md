...
Reviewed against the 2026-09-19 P1 freeze repair on HEAD `15930985f54e2e6bf4019dd0d8040796883021c7`. Live `synthetic.yml` run [35413785980](https://github.com/trannamtrung1st/agent-core/actions/runs/35413785980) on `4285c6c` failed at **Synthetic Playwright** because history/terminal seeds opened missing `data/agent-core.db` while the Synthetic host defaulted to InMemory. Local re-run of the same gate after the repair: Domain 40; Infrastructure 112/9 skip; Application 356 with blame-hang; API 123; web 328 unit tests; Playwright 34 (dedicated `data/playwright/synthetic.db`). Compose-equivalent owner-capability/volume smoke passed on isolated :5088; exact `scripts/compose-sqlite-volume.sh` was not pointed at :5080 because a Real-profile host was already listening there. Voice checklist **re-run** unedited `p1-repair-chrome-probe-run.log` (queued-text pass; combined `non-english-browser-smoke` remains fail). Unedited `p1-repair-fr-smoke-r2` `fr-FR` Browser STT/TTS pass (TTS Daniel; STT Bonjour). Earlier records stay as written: P0-2 queued-text remains machine **fail**; original `p1-final-fr-smoke` and chrome `non-english-browser-smoke` remain **fail**; `p1-final-chrome-probe-run.log` on `df0a12c` and `p1-final-fr-smoke-r2` on `5764010` are not rewritten. Optional hosted multilingual checks were not run. **P1 is frozen.** P2 has not started.
...
Authoritative contracts are in [Technology Decisions](docs/10-technology-decisions.md#decision-bounded-history-and-durable-lastentrysequence) and [Implementation Plan](docs/18-implementation-plan.md#follow-on-p1-history-lifecycle-and-multilingual-speech). P1A/P1B/P1C implementation is observed. **P1 is frozen** on HEAD `15930985f54e2e6bf4019dd0d8040796883021c7` (do not cite `4285c6c` as the verified freeze). Observed P1 replaceable speech (Browser/hosted STT/TTS independence) stays closed. Do not start P2.
