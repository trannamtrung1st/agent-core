...
**P3 freeze on `27efe17` reopened** (2026-09-21) after review of the email/approval boundary. The correction pass covers: Gmail Bcc preservation and exact-draft approval preview; MimeKit MIME plus CR/LF/NUL rejection; Gmail `drafts.send` with approved `message.raw` and draft-id guard (correction HEAD `2561167`); indeterminate/cancel-after-dispatch send consumption; approval wait isolated from 30 s/120 s execution clocks; execution policy before Gmail preview; multi-tool historical-image wire order; single owner for `waitingExternal` completion. See [docs/reports/p3-freeze-candidate.md](docs/reports/p3-freeze-candidate.md). Implementation review accepted; re-freeze P3 when hosted offline Synthetic is green on `2561167` (Compose already green). **P4** follows.
