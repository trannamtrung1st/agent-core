...
public sealed class TwentyTurnDemoTests
{
    ...
    private static async Task RunIdentityAsync(string agentId)
    {
        ...
        Assert.True(await runtime.SubmitPlaybackAsync(responseId, "started", 0, 0) is true);
        Assert.True(await runtime.SubmitPlaybackAsync(responseId, "completed", runtime.SentSamples, 0) is true);
        await runtime.WaitUntilIdleAsync();
