...
Fast development uses native .NET + Vite processes with SQLite; Docker is not required. Docker Compose is the supported reproducible integration/demo environment, using one application container and a persistent SQLite volume. See [local workflows](docs/17-observability-and-operations.md#running-after-implementation); future hybrid/on-prem Compose topologies preserve the same runtime.
