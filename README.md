...
**Planned until verified (post-MVP pack):** a concrete sandbox remains an accepted target contract. Catalog, attachments, rich responses, repeated initiative/deactivation, versioned role environments, lazy session workspaces, session-owned artifacts, and bounded typed tools (Support/Compliance workflows, OpenAI-compatible tool_calls mapping, 12/30s/120s/8 MiB caps) are observed. Historical MVP Milestones 1–12 remain the shipped baseline. See [Technology Decisions](docs/10-technology-decisions.md#post-mvp-planned-until-verified) and [Implementation Plan](docs/18-implementation-plan.md#post-mvp-phases-planned-until-verified).
