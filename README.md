...
**Planned until verified (post-MVP pack):** bounded tools and a concrete sandbox remain accepted target contracts labeled in docs 01–18. Catalog, attachments, rich responses, repeated initiative/deactivation, versioned role environments, lazy session workspaces, and session-owned artifacts (explicit materialize) are observed. Historical MVP Milestones 1–12 remain the shipped baseline. See [Technology Decisions](docs/10-technology-decisions.md#post-mvp-planned-until-verified) and [Implementation Plan](docs/18-implementation-plan.md#post-mvp-phases-planned-until-verified).
