...
- Follow one-session text/voice **mode** lifecycle (`session.mode.set`); do not create a second conversation for voice. Initialize audio from user gestures and release tracks/resources when returning to text. Mute affects input only; capture continues during output.
