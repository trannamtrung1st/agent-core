...
Read root [AGENTS.md](../../../AGENTS.md) and [README.md](../../../README.md). Infer implementation, fix, plan or review from the request; a review or plan alone does not authorize implementation, but an explicit request to review and fix authorizes corrections within the requested scope.
...
| UI layout, spacing, alignment or other visual presentation | [impeccable](../impeccable/SKILL.md) alongside frontend, with [.agents/context](../../context); `/docs` still wins on behavior |

3. Implement the smallest complete vertical change within the requested scope and milestone non-goals. For UI work, apply frontend's [component reuse and layout consistency](../frontend/SKILL.md#component-reuse-and-layout-consistency): inspect and reuse existing components/layouts first, fix spacing at its shared owner, and include affected consumers in verification. Do not resolve repeated inconsistencies with independent page-level patches. For review, trace behavior and report actionable findings with file/line evidence instead of silently fixing it.
