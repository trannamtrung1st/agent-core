...
- Validate attachment leases and sequence/offset rules. Reconnect requires Attach and an authoritative session.ready snapshot (`pendingMode` included and null after disconnect/crash recovery; session summary excluded); reject old callbacks and discard old audio, with no event/audio replay. Do not send PCM until Mode is voice.
