...
- Validate attachment leases and sequence/offset rules. Reconnect requires Attach and an authoritative session.ready snapshot (`pendingMode` included; session summary excluded); reject old callbacks and discard old audio, with no event/audio replay.
