...
## Agent Core component and layout policy

For this repository, apply frontend's [component reuse and layout consistency](../frontend/SKILL.md#component-reuse-and-layout-consistency) to UI design, implementation and review. Inspect existing product components and layouts before proposing new ones. Reuse their structure and spacing ownership; compare equivalent surfaces rather than polishing one screen in isolation.

Ant Design v6 and [.agents/context/DESIGN.md](../../context/DESIGN.md) define the presentation baseline; `/docs` owns product behavior. Generic extraction/layout playbooks must operate within that baseline: reuse AntD primitives directly and extract only justified product compositions, without adding generic control wrappers, a second UI kit or a replacement token system. Fix inconsistent gap, padding, margin and alignment in the shared owner, then verify representative consumers at the relevant widths and content states. Scope plans/reviews to recommendations unless implementation or fixes are requested. Plans specify verification without running the app; runnable reviews inspect existing behavior. Follow frontend/testing fallbacks when execution is unavailable.

The bounded polishing rounds limit discretionary visual iteration; they do not waive [required runtime verification](../testing/SKILL.md#runtime-verification), regression checks or fixing failures caused by the change. Batch visual checks across representative consumers and report remaining gaps honestly.
